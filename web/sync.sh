#!/bin/bash
# Non-interactive dist sync for the player-facing site.
# Usage: ./sync.sh <host> <remote_dir>
#   e.g. ./sync.sh doudou-prod /data/dl/web
# remote_dir has no safe default (unlike gameadmin/web) - the exact production
# path was not established in this repo, so it must always be passed explicitly.
set -euo pipefail
